...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), JetBrains Mono for all
  display + JetBrains Mono body + JetBrains Mono for the index/code voice on a warm-navy code surface,
...
colors:
  ink: "#070a0f"           # 最暗面（コードサーフェス / インセット背景）
  cream: "#d6deeb"         # 前景テキスト（LP-A の --fg）
  muted: "#7d8ba0"         # 二次テキスト（--fg-muted）
  dim: "#4a5568"           # 三次テキスト・コメント（--fg-dim）
  tile: "#0f151f"          # 面（--bg-elev。淡いタイル背景）
  tile-strong: "#1c2530"   # 境界・強調タイル（--border）
  coral: "#3fb950"         # ★主 voltage = retrace の緑（CTA・進捗・+追加行）
  coral-bright: "#56d364"  # 緑ハイライト（--green-bright）
  amber: "#d29922"         # 「何を/なぜ」解説ラベル用（LP の t-hl）
  cyan: "#39c5cf"          # メタ情報（sha・ファイル数）
  red: "#f85149"           # 削除行・課題ラベル（-削除）
  navy: "#0a0e14"          # 基本キャンバス（LP-A の --bg）
  navy-soft: "#0f151f"
  navy-elev: "#141a24"
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "JetBrains Mono", cqw: 1.5, weight: 400, lineHeight: 1.5 }
  lead:    { fontFamily: "JetBrains Mono", cqw: 2.08, weight: 400, lineHeight: 1.5 }
  card-title:{ fontFamily: "JetBrains Mono", cqw: 2.3, weight: 500, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "JetBrains Mono", cqw: 1.46, weight: 500, lineHeight: 1.0 }
  ...
  # — display ramp (JetBrains Mono 400, sentence case, negative tracking. Renderer embeds only 400/700 — author at 400; italic is the synthesized slant) —
  headline:{ fontFamily: "JetBrains Mono", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "JetBrains Mono", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "JetBrains Mono", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "JetBrains Mono", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "JetBrains Mono", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "JetBrains Mono", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.028em" }
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a JetBrains Mono figure paired with a JetBrains Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (JetBrains Mono italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. JetBrains Mono italic, small JetBrains Mono uppercase cite beneath."
  section-rule:
...
- **Fonts** — already set to **JetBrains Mono** (display) / **ui-monospace** (body); ignore any preset font name lingering in prose.
- **Colors** — use the frontmatter hex; preset color NAMES in prose (e.g. "cobalt", "cream") mean the remapped brand values.

### ⚠️ retrace は DARK-FIRST — preset の light 前提を反転して読む（最重要）

この preset の原文は「cream 地・ink 文字・navy はコードだけ」という **light 基調**を語るが、retrace（と埋め込み先 LP-A）は **逆に dark が全面の地**。以下で読み替える：

- **地は常に `{colors.navy}` #0a0e14（＋うっすらグリッド）**。cream/tile を「地」とする記述は無視。`{colors.cream}` #d6deeb は**文字色**、`{colors.navy-soft}`/`{colors.navy-elev}` が面・カード。
- **`coral` = retrace の緑 #3fb950 が主 voltage**。CTA・進捗・diff の `+追加行`・成功の合図。原文の「coral は1フレームに一度だけ」は緩めてよい（緑は retrace の常在アクセント）。ただし濫用はせず、意味のある場所（CTA/追加/進捗）に置く。
- **診断的ハイライト2色は役割固定**：`amber` #d29922 =「何を/なぜ」の解説ラベル、`cyan` #39c5cf = メタ（sha・ファイル数）、`red` #f85149 = diff の `-削除行`・課題ラベル。これらは装飾ではなく LP-A の意味づけをそのまま踏襲する。
- **ターミナル/コードサーフェスは "特別な暗い面" ではなく、動画全体のトーンそのもの**。信号機ドット + `retrace — <owner>/<repo> · commit #N` のタイトルバーを持つターミナルカードは retrace の象徴。積極的に使う。
- コントラスト：緑/シアン/アンバー/赤いずれも #0a0e14 上で AA を満たす。文字本文は必ず `{colors.cream}`、弱い文字は `{colors.muted}`/`{colors.dim}`。
...
Three editorial voices, each in its own face: **JetBrains Mono** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **JetBrains Mono** carries body, leads, card titles, buttons, and
...
- **JetBrains Mono** (sentence case, negative-tracked) for all display; **JetBrains Mono** body/chrome; **JetBrains Mono** index + code.
...
- **Squint** — one JetBrains Mono display moment dominates at 3–6× its neighbor.
...
- **Type** — JetBrains Mono sentence-case display (negative-tracked); JetBrains Mono 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (JetBrains Mono `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (JetBrains Mono `headline` 4.6cqw
...
- **JetBrains Mono display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **JetBrains Mono body** sentence case weight 400. **JetBrains Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — JetBrains Mono figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — JetBrains Mono italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized JetBrains Mono · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line JetBrains Mono `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** JetBrains Mono 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single JetBrains Mono line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line JetBrains Mono `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, number-lockup, lead/caption, optional `section-rule`. **Focal** a JetBrains Mono `number-hero` figure with a mono unit over a 1px rule — the PR impact (`+1,204 / −318`, `17 files`, `2.1× faster`). **Chrome** mono tag. **Accent** the figure in ink; at most one coral unit. **Fixed** serif figure + mono unit, hairline rule. **Free** the figures (from the script), tag, layout + density. **Density** free.

### 5 · Pull-quote (quote · move: JetBrains Mono italic · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, pull-quote, tag-upper cite. **Focal** a JetBrains Mono **italic** `quote-pull` — a commit message, a reviewer line, or the thesis — with a small JetBrains Mono uppercase cite (author · role) beneath. **Chrome** mono kicker. **Accent** none, or one coral mark. **Fixed** JetBrains Mono italic quote + uppercase cite. **Free** quote, attribution, layout + density. **Density** free.

### 6 · Closing / CTA (closer · move: coral voltage · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}`). **Composes** display sign-off, coral-callout, optional contributor row (`assets/<login>.png` avatars + mono names). **Focal** a short JetBrains Mono sign-off with the one **coral-callout** (the CTA or full-bleed band) and, for a "shipped-by" close, a row of hairline-ringed avatar chips. **Chrome** mono index. **Accent** the single coral voltage. **Fixed** one coral moment, hairline avatar rings, sentence-case serif. **Free** sign-off, who ships, layout + density. **Density** free.
...
- Set all display in **JetBrains Mono, sentence case**, negative-tracked; **JetBrains Mono 400** body; **JetBrains Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a JetBrains Mono figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case JetBrains Mono display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one JetBrains Mono display moment dominates.
...
- **Type** — JetBrains Mono sentence-case display (negative-tracked); JetBrains Mono 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
